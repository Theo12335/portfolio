...
// NEW: Add props for onOpenModal and onCloseModal
interface CarouselProps {
  onOpenModal: () => void;
  onCloseModal: () => void;
}

// Update the Carousel component to accept the new props
const Carousel: React.FC<CarouselProps> = ({ onOpenModal, onCloseModal }) => {
  ...
  // Effect to update slide position and manage auto-cycle
  useEffect(() => {
    if (trackRef.current) {
      trackRef.current.style.transform = `translateX(-${currentIndex * 100}%)`;
    }

    startAutoCycle(); // Start auto-cycle whenever currentIndex changes or component mounts

    return () => {
      stopAutoCycle(); // Clean up interval on component unmount
    };
  }, [currentIndex, slideCount, startAutoCycle, stopAutoCycle]);

  // Project Modal State and Handlers
  ...
  const openModal = (project: Project) => {
    ...
    setIsModalOpen(true);
    stopAutoCycle(); // Stop auto-cycle when modal is open
    onOpenModal(); // NOTIFY PARENT: Modal is opening
  };

  const closeModal = () => {
    setIsModalOpen(false);
    setSelectedProject(null);
    startAutoCycle(); // Resume auto-cycle when modal is closed
    onCloseModal(); // NOTIFY PARENT: Modal is closing
  };
  ...
  return (
    <>
      <div className="relative w-full max-w-4xl mx-auto overflow-hidden rounded-lg pointer-events-auto">
        ...
        >
          {projectsData.map((project) => (
            ...
            >
              <div className="relative w-full h-[400px] mb-4 max-h-96 transition-all duration-300">
                <Image
                  src={project.image}
                  alt={project.title}
                  layout="fill"
      ...
      {isModalOpen && selectedProject && (
        // Pass closeModal from Carousel as onClose to ProjectModal (which in turn calls Carousel's closeModal)
